...
// components
import GenderBox from "../GenderBox/GenderBox";
...
export default function FixedBox() {
  ...
  return (
    <>
      <span className="fixedBoxIcons" onClick={() => setModal(!modal)}>
        {contactLists.length <= 0 ? null : (
          <span className="fixedBoxLenght">{contactLists?.length}</span>
        )}
        <FaBookmark />
      </span>
      {/* <span
...
const ModalLists = ({ setModal }) => {
  ...
  return (
    <div className="modalListsBox">
      <div className="modalTitleBox">
        <h2>Contact Lists</h2>
        <span className="modalIconClose" onClick={() => setModal(false)}>
          <MdClose />
        </span>
      </div>
      {contactLists <= 0 ? (
        <p style={{ textAlign: "center" }}>ยังไม่มีรายการ</p>
      ) : (
        <ul>
          {contactLists?.map((item, index) => {
            return (
              <li key={index}>
                ...
                <p>
                  Text : <b>{item.message}</b>
                </p>
                <span onClick={() => dispatch(deleteContact({ id: item.id }))}>
                  <FaTrashAlt />
                </span>
              </li>
            );
